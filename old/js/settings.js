...
import { makeSeperator } from "./utils.js";
...
function fillSettingsList({
  ...
  if (referenced.size === 0) {
    ...
    root.after(makeSeperator());
  }

  const Wrap = document.createElement("div");
  Wrap.className = "col small";
  Wrap.id = "SettingsContainer";
...
// Stages
function fillSettingsStage(targetId = "settings_stage", headerKey = "app.settings.STAGE.header", contentKey = "app.settings.STAGE.content") {
  fillSettingsList({ targetId, prop: "stage", enumObj: STAGE_ENUM, headerKey, contentKey });
}
...
// Intensity
function fillSettingsIntensity(targetId = "settings_intensity", headerKey = "app.settings.INTENSITY.header", contentKey = "app.settings.INTENSITY.content") {
  fillSettingsList({ targetId, prop: "intensity", enumObj: INTENSITY_ENUM, headerKey, contentKey });
}
...
// Extremity
function fillSettingsExtremity(targetId = "settings_extremity", headerKey = "app.settings.EXTREMITY.header", contentKey = "app.settings.EXTREMITY.content") {
  fillSettingsList({ targetId, prop: "extremity", enumObj: EXTREMITY_ENUM, headerKey, contentKey });
}
...
// Act With
function fillSettingsActWith(targetId = "settings_act_with", headerKey = "app.settings.ACT_WITH.header", contentKey = "app.settings.ACT_WITH.content") {
  fillSettingsList({ targetId, prop: "act_with", enumObj: ACT_WITH_ENUM, headerKey, contentKey });
}
...
// Act On
function fillSettingsActOn(targetId = "settings_act_on", headerKey = "app.settings.ACT_ON.header", contentKey = "app.settings.ACT_ON.content") {
  fillSettingsList({ targetId, prop: "act_on", enumObj: ACT_ON_ENUM, headerKey, contentKey });
}
